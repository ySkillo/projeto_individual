...
function comentarioRealizado(idPostagem) {
    ...
    var instrucaoSql = `
        ...
        p.idPostagem, u.nomePerfilUsuario, u.idUsuario, u.fotoPerfil FROM tbComentarios as c 
...
//deletando comentario para apagar postagem
function apagarComentariosDaPostagem(idPostagem) {
    console.log("ACESSEI O USUARIO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function apagandoComentario():");
    
    // Insira exatamente a query do banco aqui, lembrando da nomenclatura exata nos valores
    //  e na ordem de inserção dos dados.
    var instrucaoSql = `
        DELETE FROM tbcomentarios WHERE fkPostagem = ${idPostagem};
    `;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
